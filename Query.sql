SELECT Jobs.*, JobCategories.*, JobTypes.*, Jobs.id as Jobs__id
 ...
 FROM jobs Jobs
    ...
    INNER JOIN job_categories JobCategories
        ON (JobCategories.id = (Jobs.job_category_id) AND (JobCategories.deleted) IS NULL) INNER JOIN job_types JobTypes
        ON (JobTypes.id = (Jobs.job_type_id) AND (JobTypes.deleted) IS NULL)

    
    # This part can be use Full Text Search
    # ALTER TABLE Jobs ADD FULLTEXT (Jobs.name, Jobs.description, detail, business_skill, jobs_practical_skills.name, basic_abilities.name);

    WHERE MATCH(Jobs.name, 
                Jobs.description, 
                Jobs.detail, 
                Jobs.business_skill, 
                Jobs.knowledge, 
